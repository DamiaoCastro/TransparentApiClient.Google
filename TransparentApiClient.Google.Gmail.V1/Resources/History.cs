...
namespace TransparentApiClient.Google.Gmail.V1.Resources { 
	...
	public class History : BaseClient {
		...
		public Task<BaseResponse<Schema.ListHistoryResponse>> ListAsync(string userId, string historyTypes, string labelId, int? maxResults, string pageToken, string startHistoryId, JsonSerializerSettings settings = null, CancellationToken cancellationToken = default(CancellationToken)) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {historyTypes, labelId, maxResults, pageToken, startHistoryId});

			return SendAsync(HttpMethod.Get, $"{userId}/history?{queryString}", null, settings, cancellationToken)
