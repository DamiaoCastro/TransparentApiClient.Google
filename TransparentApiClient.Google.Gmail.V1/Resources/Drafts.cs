...
namespace TransparentApiClient.Google.Gmail.V1.Resources { 
	...
	public class Drafts : BaseClient {
		...
		public Task<BaseResponse<Schema.Draft>> CreateAsync(string userId, Schema.Draft Draft, JsonSerializerSettings settings = null, CancellationToken cancellationToken = default(CancellationToken)) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/drafts", Draft, settings, cancellationToken)
		...
		public Task<BaseResponse<object>> DeleteAsync(string id, string userId, JsonSerializerSettings settings = null, CancellationToken cancellationToken = default(CancellationToken)) {
			...
			return SendAsync(HttpMethod.Delete, $"{userId}/drafts/{id}", null, settings, cancellationToken)
		...
		public Task<BaseResponse<Schema.Draft>> GetAsync(string id, string userId, string format, JsonSerializerSettings settings = null, CancellationToken cancellationToken = default(CancellationToken)) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {format});

			return SendAsync(HttpMethod.Get, $"{userId}/drafts/{id}?{queryString}", null, settings, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Draft>, cancellationToken)
				.Unwrap();
		}
		...
		public Task<BaseResponse<Schema.ListDraftsResponse>> ListAsync(string userId, bool? includeSpamTrash, int? maxResults, string pageToken, string q, JsonSerializerSettings settings = null, CancellationToken cancellationToken = default(CancellationToken)) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {includeSpamTrash, maxResults, pageToken, q});

			return SendAsync(HttpMethod.Get, $"{userId}/drafts?{queryString}", null, settings, cancellationToken)
		...
		/// </summary>
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Message>> SendAsync(string userId, Schema.Draft Draft, JsonSerializerSettings settings = null, CancellationToken cancellationToken = default(CancellationToken)) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/drafts/send", Draft, settings, cancellationToken)
		...
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Draft>> UpdateAsync(string id, string userId, Schema.Draft Draft, JsonSerializerSettings settings = null, CancellationToken cancellationToken = default(CancellationToken)) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Put, $"{userId}/drafts/{id}", Draft, settings, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Draft>, cancellationToken)
				.Unwrap();
		}
