...
namespace TransparentApiClient.Google.Gmail.V1.Resources { 
	...
	public class Messages : BaseClient {
		...
		public Task<BaseResponse<object>> BatchDeleteAsync(string userId, Schema.BatchDeleteMessagesRequest BatchDeleteMessagesRequest, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/messages/batchDelete", BatchDeleteMessagesRequest, cancellationToken)
		...
		/// </summary>
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<object>> BatchModifyAsync(string userId, Schema.BatchModifyMessagesRequest BatchModifyMessagesRequest, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/messages/batchModify", BatchModifyMessagesRequest, cancellationToken)
				.ContinueWith(HandleBaseResponse<object>, cancellationToken)
				.Unwrap();
		}
		...
		public Task<BaseResponse<object>> DeleteAsync(string id, string userId, CancellationToken cancellationToken) {
			...
			return SendAsync(HttpMethod.Delete, $"{userId}/messages/{id}", null, cancellationToken)
				.ContinueWith(HandleBaseResponse<object>, cancellationToken)
				.Unwrap();
		}
		...
		public Task<BaseResponse<Schema.Message>> GetAsync(string id, string userId, string format, string metadataHeaders, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {format, metadataHeaders});

			return SendAsync(HttpMethod.Get, $"{userId}/messages/{id}?{queryString}", null, cancellationToken)
		...
		public Task<BaseResponse<Schema.Message>> ImportAsync(string userId, bool? deleted, string internalDateSource, bool? neverMarkSpam, bool? processForCalendar, Schema.Message Message, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {deleted, internalDateSource, neverMarkSpam, processForCalendar});

			return SendAsync(HttpMethod.Post, $"{userId}/messages/import?{queryString}", Message, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Message>, cancellationToken)
				.Unwrap();
		}
		...
		/// <param name="deleted">Mark the email as permanently deleted (not TRASH) and only visible in Google Vault to a Vault administrator. Only used for G Suite accounts.</param>
		/// <param name="internalDateSource">Source for Gmail's internal date of the message.</param>
		public Task<BaseResponse<Schema.Message>> InsertAsync(string userId, bool? deleted, string internalDateSource, Schema.Message Message, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {deleted, internalDateSource});

			return SendAsync(HttpMethod.Post, $"{userId}/messages?{queryString}", Message, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Message>, cancellationToken)
				.Unwrap();
		}
		...
		public Task<BaseResponse<Schema.ListMessagesResponse>> ListAsync(string userId, bool? includeSpamTrash, string labelIds, int? maxResults, string pageToken, string q, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {includeSpamTrash, labelIds, maxResults, pageToken, q});

			return SendAsync(HttpMethod.Get, $"{userId}/messages?{queryString}", null, cancellationToken)
		...
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Message>> ModifyAsync(string id, string userId, Schema.ModifyMessageRequest ModifyMessageRequest, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/messages/{id}/modify", ModifyMessageRequest, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Message>, cancellationToken)
				.Unwrap();
		}
		...
		/// </summary>
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Message>> SendAsync(string userId, Schema.Message Message, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/messages/send", Message, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Message>, cancellationToken)
				.Unwrap();
		}
		...
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Message>> TrashAsync(string id, string userId, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/messages/{id}/trash", null, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Message>, cancellationToken)
				.Unwrap();
		}
		...
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Message>> UntrashAsync(string id, string userId, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/messages/{id}/untrash", null, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Message>, cancellationToken)
				.Unwrap();
		}
