...
namespace TransparentApiClient.Google.Gmail.V1.Resources { 
	...
	public class Threads : BaseClient {
		...
		public Task<BaseResponse<object>> DeleteAsync(string id, string userId, CancellationToken cancellationToken) {
			...
			return SendAsync(HttpMethod.Delete, $"{userId}/threads/{id}", null, cancellationToken)
		...
		public Task<BaseResponse<Schema.Thread>> GetAsync(string id, string userId, string format, string metadataHeaders, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {format, metadataHeaders});

			return SendAsync(HttpMethod.Get, $"{userId}/threads/{id}?{queryString}", null, cancellationToken)
		...
		public Task<BaseResponse<Schema.ListThreadsResponse>> ListAsync(string userId, bool? includeSpamTrash, string labelIds, int? maxResults, string pageToken, string q, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			string queryString = GetQueryString(new {includeSpamTrash, labelIds, maxResults, pageToken, q});

			return SendAsync(HttpMethod.Get, $"{userId}/threads?{queryString}", null, cancellationToken)
		...
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Thread>> ModifyAsync(string id, string userId, Schema.ModifyThreadRequest ModifyThreadRequest, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/threads/{id}/modify", ModifyThreadRequest, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Thread>, cancellationToken)
				.Unwrap();
		}
		...
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Thread>> TrashAsync(string id, string userId, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/threads/{id}/trash", null, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Thread>, cancellationToken)
				.Unwrap();
		}
		...
		/// <param name="userId">The user's email address. The special value me can be used to indicate the authenticated user.</param>
		public Task<BaseResponse<Schema.Thread>> UntrashAsync(string id, string userId, CancellationToken cancellationToken) {
			if (string.IsNullOrWhiteSpace(id)) { throw new ArgumentNullException(nameof(id)); }
			if (string.IsNullOrWhiteSpace(userId)) { throw new ArgumentNullException(nameof(userId)); }

			return SendAsync(HttpMethod.Post, $"{userId}/threads/{id}/untrash", null, cancellationToken)
				.ContinueWith(HandleBaseResponse<Schema.Thread>, cancellationToken)
				.Unwrap();
		}
